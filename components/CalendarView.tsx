...
const CalendarView: React.FC<CalendarViewProps> = ({ events, onTaskDrop }) => {
  ...
  const handleDrop = (e: React.DragEvent, date: Date) => {
      ...
      const relativeY = (e.nativeEvent as any).offsetY;
                                ...
                                return (
                                    <div key={event.id} style={style} className={className} title={event.title}>
                                        <div className="flex items-center gap-1 overflow-hidden">
                                            {event.recurrence && event.recurrence.frequency !== 'none' as any && (
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-2.5 h-2.5 shrink-0 opacity-70">
                                                    <path fillRule="evenodd" d="M15.312 11.424a5.5 5.5 0 01-9.201 2.466l-.312-.311h2.433a.75.75 0 000-1.5H3.984a.75.75 0 00-.75.75v4.248a.75.75 0 001.5 0v-2.433l.312.312a7 7 0 0011.714-3.147.75.75 0 00-1.448-.386zm-.312-3.848a7 7 0 00-11.714 3.147.75.75 0 001.448.386 5.5 5.5 0 019.201-2.466l.312.311h-2.433a.75.75 0 000 1.5H16.016a.75.75 0 00.75-.75V3.984a.75.75 0 00-1.5 0v2.433l-.312-.312z" clipRule="evenodd" />
                                                </svg>
                                            )}
                                            <div className="font-bold truncate leading-none text-shadow-sm">{event.title}</div>
                                        </div>
